...
CSS의 `!important`도, Tailwind의 `!` 접두사(`!` + 유틸리티명)도 쓰지 않습니다.
예외 없습니다.

> **문서에 Tailwind 클래스를 예시로 적을 때 주의하세요.** Tailwind v4는 `.md`
> 파일도 스캔합니다. 이 문서에 `!` 접두사 예시를 **실제 클래스 이름으로**
> 적었더니 그 유틸리티가 프로덕션 CSS에 그대로 생성됐습니다. 금지 규칙을
> 적는 행위가 금지 대상을 배포한 셈입니다. 예시는 `bg-…`처럼 컴파일되지 않는
> 형태로 쓰세요.

덮어쓰기가 안 될 때 쓰라고 있는 것처럼 보이지만, 실제로는 **캐스케이드를
이해하지 못했다는 신호**입니다. 한 번 쓰면 그것을 이기려고 다음 사람이 또
쓰게 되고, 결국 어느 규칙이 이기는지 아무도 모르게 됩니다.

### 왜 필요 없는가

Tailwind v4는 유틸리티를 `@layer utilities` 안에 넣습니다.

```
@layer properties | @layer theme | @layer base | @layer components | @layer utilities
```

캐스케이드 레이어 규칙상 **레이어에 속하지 않은 선언이 레이어 안 선언을
이깁니다.** 특정도와 무관합니다. 즉 유틸리티를 덮어써야 하면 `@layer` 밖에
평범한 규칙을 하나 두면 끝입니다.

`styles/globals.css`의 `prefers-reduced-motion` 블록이 그 예입니다. `@layer base`
안에 있을 때는 base가 utilities보다 약해서 `!important` 4개가 필요했지만,
레이어 밖으로 옮기자 전부 없앨 수 있었습니다.

### 순서대로 시도하세요

| 순서 | 방법 | 언제 |
|---|---|---|
| 1 | 컴포넌트에 `className`을 넘긴다 | 대부분의 경우. `cn()`이 충돌을 병합합니다 |
| 2 | `@layer` 밖의 일반 규칙 | 전역 조건부 덮어쓰기 (`prefers-reduced-motion`, `noscript`) |
| 3 | 토큰·variant 자체를 고친다 | 같은 덮어쓰기를 두 번 이상 하게 될 때 |

세 가지로 안 되면 **보고하고 멈추세요.**
