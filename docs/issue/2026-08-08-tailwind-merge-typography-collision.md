...
2. **토큰 목록 일치를 검사하는 테스트** — `globals.css`를 읽어 `--text-*`를 뽑고
   `fontSizeTokens`와 비교하는 테스트 한 개면 위 위험이 거의 사라집니다.
   **비용이 낮아 다음 작업 때 추가할 가치가 있습니다.**
...
- **토큰 목록 이중 관리.** 새 타이포 토큰 추가 시 `lib/utils.ts`를 함께 고쳐야 합니다.
  위 "더 나은 방법 2"의 동기화 테스트를 넣기 전까지는 사람의 주의에 의존합니다.
