...
특히 이 에이전트가 주의할 것: **클래스가 조용히 사라지는 문제.** `cn()`은
tailwind-merge 기반이라 커스텀 토큰을 잘못된 그룹으로 분류하면 한쪽이 다른 쪽을
지웁니다. 실제 사례는 `docs/issue/2026-08-08-tailwind-merge-typography-collision.md`에
있습니다. **타이포 토큰을 새로 추가할 때는 `styles/globals.css`와 `lib/utils.ts`를
반드시 함께 고치세요.** 한쪽만 고치면 빌드는 통과하고 화면만 틀립니다.
